...
if($result->num_rows > 0){
    ...
    foreach($result as $row){
        ...
        <tr "; if($row['povtor_lead']==1){ echo 'class="duble"'; }  echo ">
            ...
            $ghgj = $row['phone'];
            echo "</td><td><div class='calls_lead' data-phone='".$ghgj."'><img src='https://san-dez.ru/my_script/Mango/outcall.png'> ";
            
            $sqlCall = "SELECT * FROM calls WHERE call_from = '".$ghgj."' || call_to = '".$ghgj."'";
            $resultCall = $conn->query($sqlCall);
            echo $resultCall->num_rows;
            echo "</div></td>
            ...
            <td>".$marzha."</td>
            <td>".$row['name_leads']; if($row['dialogId']!= 'В WhatsApp не существует пользователя с таким номером'){echo "<button class='but_wz_sent' data-id_crm='".$row['id_crm']."' id='".$row['dialogId']."' data-phone='".$row['phone']."' data-chat_id_max='".$row['chat_id_max']."'  data-messenger_activ='".$messenger_activ."'>Написать</button>";} if($row['dialog_wiev']==1){echo '<img class="new_mes" src="img/whatsapp_icon.png">';} echo "</td>
            <td>".$row['name']."</td>
            <td>".$row['mail']."</td>
    ...
    <footer>
      <div class="block_mes_wz">
        ...
        <div class="vibor_messenger_block">
            <div><p class="wz_name">Ирина Золотарёва</p></div>
...
Ато наш начальник потом мне не говорил что почему до сехпор есть клопов.</p>
    ...
    <script src="app_Max.js?v=4" defer></script> 
