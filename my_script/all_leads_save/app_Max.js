...
//setTimeout(() => { clearInterval(timerId); alert('stop'); }, 5000);
    ...
    $('body').on('click', '.new_mes_noti', function(){
        ...
        if($(this).attr('data-id_dialog')!=''){showmessageTeletype($(this).attr('data-id_dialog'));}else{showmessage($(this).attr('data-phone'));}
    ...
    $('.but_mes_sent').on('click', function(){
        ...
        if($('.wz_dialogId').val()!=''){
            ...
            var chat_id_max = $('.wz_chat_id_max').val();
            $('.mes_wz').html('<img src="https://san-dez.ru/my_script/Teletype/load_icon.gif">');
            
            if($('.wz_messenger_activ').val() == 'max'){var url_script = 'https://san-dez.ru/my_script/wappi_Max/Api_Max.php';}
            if($('.wz_messenger_activ').val() == 'wz'){var url_script = 'https://san-dez.ru/my_script/wappi/api.php';}
            ...
            $.get(url_script, {type_mes: 'read', id_mes: $('.mes_wz .mes-client').attr('id'), phone:$('.wz_phone').html()}, function(data){});
    ...
    $('.view_mes').on('click', function(){ 
        ...
        if($('.wz_messenger_activ').val() == 'max'){var url_script = 'https://san-dez.ru/my_script/wappi_Max/Api_Max.php';}
        if($('.wz_messenger_activ').val() == 'wz'){var url_script = 'https://san-dez.ru/my_script/wappi/api.php';}
        $.get(url_script, {type_mes: 'read', id_mes: $('.mes_wz .mes-client').attr('id'), phone:$('.wz_phone').html(), chat_id:$('.wz_chat_id_max').val()}, function(data){//alert(data);
            setTimeout(() => location.reload(), 2000);
        });
    });
