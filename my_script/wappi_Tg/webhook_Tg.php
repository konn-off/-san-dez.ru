...
function writeToLog($data, $title = '') {
    ...
    $log .= print_r ($data,1);
    $log .= "\n------------------------\n";
    file_put_contents(getcwd() . '/hookTG.log', $log, FILE_APPEND);
    return true;
}
...
sent_Loop('Есть ХукТГ - '.$data['type'].', '.$data['from'].' - '.$data['body'],0);
...
//if($phone!=''){
    ...
    $sql2 = "SELECT * FROM all_leads WHERE chat_id_tg = $chat_id_tg";
    if($conn->query($sql2)){
    ...
    $sql3 = "SELECT * FROM all_leads WHERE chat_id_tg = $chat_id_tg LIMIT 1";
    if($conn->query($sql3)){
        ...
        if($result->num_rows != 0){ 
            ...
            $sql4 = "SELECT * FROM all_leads WHERE chat_id_tg = $chat_id_tg ORDER BY id DESC LIMIT 1";
            if($conn->query($sql4)){
    ...
    */    
    $id_crm = '';    
        ...
        $name_data = "(date, name_leads, name, mail, phone, site, url, roistat, ycid, utm_source, utm_medium, utm_campaign, utm_content, utm_term, source, povtor_lead, id_crm, dialogId, dialog_wiev, last_mes, last_mes_time, chat_id_tg, last_messenger, notise_audio)";
        $data = "('".$date."', '".$name_leads."', '".$name."', '".$mail."', '".$phone."', '".$site."', '".$url."', '".$roistat."', '".$yclid."', '".$utm_source."', '".$utm_medium."', '".$utm_campaign."', '".$utm_content."', '".$utm_term."', '".$source."', '".$povtor_lead."', '".$id_crm."', '".$dialogId."', 1, '".$text_mess."', '".$date."', '".$chat_id_tg."', 'tg', 1)";
        ...
        if($conn->query($sql)){ $messageMy = "Данные успешно добавлены"; 
            ...
            if($id_click_tg){
                $sql4 = "UPDATE click_max SET id_lead = $id WHERE id = '".$id_click_tg."'"; 
                $conn->query($sql4);
            }
        ...
        if($yclid != ''){
            ...
            if($povtor_lead==0){
                ...
                sent_Loop("Конверсия в Метрику TG: " .$yaInfo,0);
            }
        }
        ...
        if($povtor_lead_nedavno==0){
            $name_data = "(date, name_leads, name, mail, phone, site, url, roistat, ycid, utm_source, utm_medium, utm_campaign, utm_content, utm_term, source, povtor_lead, id_crm, dialogId, dialog_wiev, chat_id_tg, last_messenger, notise_audio)";
            $data = "('".$date."', '".$name_leads."', '".$name."', '".$mail."', '".$phone."', '".$site."', '".$url."', '".$roistat."', '".$yclid."', '".$utm_source."', '".$utm_medium."', '".$utm_campaign."', '".$utm_content."', '".$utm_term."', '".$source."', '".$povtor_lead."', '".$id_crm."', '".$dialogId."', 1, '".$chat_id_tg."', 'tg', 1)";
                        
            $sql = "INSERT INTO all_leads $name_data VALUES $data"; 
            if($conn->query($sql)){ /*echo "Данные успешно добавлены";*/ } else { sent_Loop("Ошибка: " .$conn->error,0); /*send_notification2 ("Ошибка: " .$conn->error);*/ }
            
            $sql4 = "UPDATE all_leads SET dialog_wiev = 1, last_mes = '".$text_mess."', last_mes_time = '".$date."', notise_audio = 1 WHERE chat_id_tg = '".$chat_id_tg."'"; 
            $conn->query($sql4);
        }
        if($phone !='' ){ $fffs = 'phone = '.$phone.',';}else{ $fffs = ''; }
        $sql4 = "UPDATE all_leads SET dialog_wiev = 1,".$fffs." last_mes = '".$text_mess."', last_mes_time = '".$date."', notise_audio = 1 WHERE chat_id_tg = '".$chat_id_tg."'"; 
        $conn->query($sql4);
    }
